	...
	public class ProcButtons {
		...
		public static void FillCache(DataTable table) {
			...
			for(int i=0;i<table.Rows.Count;i++) {
				...
				List[i].ProcButtonNum=PIn.PLong(table.Rows[i][0].ToString());
				List[i].Description=PIn.PString(table.Rows[i][1].ToString());
				List[i].ItemOrder=PIn.PInt(table.Rows[i][2].ToString());
				List[i].Category=PIn.PLong(table.Rows[i][3].ToString());
		...
		public static long Insert(ProcButton but) {
			...
			if(PrefC.RandomKeys) {
				command+=POut.PLong(but.ProcButtonNum)+", ";
			}
			command+=
				 "'"+POut.PString(but.Description)+"', "
				+"'"+POut.PLong   (but.ItemOrder)+"', "
				+"'"+POut.PLong   (but.Category)+"', "
		...
		public static void Update(ProcButton but) {
			...
			string command="UPDATE procbutton SET " 
				...
				+ ",ItemOrder = '"  +POut.PLong   (but.ItemOrder)+"'"
				+ ",Category = '"   +POut.PLong   (but.Category)+"'"
				+ ",ButtonImage = '"+POut.PBitmap(but.ButtonImage)+"'"
				+" WHERE ProcButtonNum = '"+POut.PLong(but.ProcButtonNum)+"'";
			Db.NonQ(command);
		}
