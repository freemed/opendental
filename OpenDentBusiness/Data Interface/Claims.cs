...
namespace OpenDentBusiness{
	...
	public class Claims{
		...
		public static List<ClaimPaySplit> RefreshByCheck(long claimPaymentNum,bool showUnattached) {
			...
			string command=
				...
				+" AND (claimproc.Status = '1' OR claimproc.Status = '4' OR claimproc.Status=5)"//received or supplemental or capclaim
 				+" AND (claimproc.ClaimPaymentNum = '"+POut.PLong(claimPaymentNum)+"'";
			if(showUnattached){
			...
			for(int i=0;i<table.Rows.Count;i++){
				...
				split.ProvAbbr       =Providers.GetAbbr(PIn.PLong(table.Rows[i]["ProvTreat"].ToString()));
				...
				split.ClaimNum       =PIn.PLong   (table.Rows[i]["ClaimNum"].ToString());
				split.ClaimPaymentNum=PIn.PLong   (table.Rows[i]["ClaimPaymentNum"].ToString());
				split.PatNum         =PIn.PLong   (table.Rows[i]["PatNum"].ToString());
				splits.Add(split);
			}
			return splits;
		}
		...
		public static Claim GetClaim(long claimNum) {
			...
			command="SELECT * FROM claimattach WHERE ClaimNum = "+POut.PLong(claimNum);
			table=Db.GetTable(command);
			retClaim.Attachments=new List<ClaimAttach>();
			ClaimAttach attach;
			for(int i=0;i<table.Rows.Count;i++){
				attach=new ClaimAttach();
				attach.ClaimAttachNum   =PIn.PLong   (table.Rows[i][0].ToString());
				attach.ClaimNum         =PIn.PLong   (table.Rows[i][1].ToString());
		...
		private static List<Claim> SubmitAndFill(DataTable table){
			...
			for(int i=0;i<table.Rows.Count;i++){
				tempClaim=new Claim();
				tempClaim.ClaimNum     =		PIn.PLong   (table.Rows[i][0].ToString());
				tempClaim.PatNum       =		PIn.PLong   (table.Rows[i][1].ToString());
				...
				tempClaim.PlanNum      =		PIn.PLong   (table.Rows[i][6].ToString());
				tempClaim.ProvTreat    =		PIn.PLong   (table.Rows[i][7].ToString());
				...
				tempClaim.ProvBill     =		PIn.PLong   (table.Rows[i][18].ToString());
				tempClaim.ReferringProv=		PIn.PLong   (table.Rows[i][19].ToString());
				tempClaim.RefNumString =		PIn.PString(table.Rows[i][20].ToString());
				tempClaim.PlaceService = (PlaceOfService)PIn.PLong(table.Rows[i][21].ToString());
				...
				tempClaim.EmployRelated=(YN)PIn.PLong   (table.Rows[i][25].ToString());
				...
				tempClaim.PatRelat      =(Relat)PIn.PLong(table.Rows[i][29].ToString());
				tempClaim.PlanNum2      =   PIn.PLong   (table.Rows[i][30].ToString());
				tempClaim.PatRelat2     =(Relat)PIn.PLong(table.Rows[i][31].ToString());
				tempClaim.WriteOff      =   PIn.PDouble(table.Rows[i][32].ToString());
				tempClaim.Radiographs   =   PIn.PInt   (table.Rows[i][33].ToString());
				tempClaim.ClinicNum     =   PIn.PLong   (table.Rows[i][34].ToString());
				tempClaim.ClaimForm     =   PIn.PLong   (table.Rows[i][35].ToString());
				tempClaim.EFormat       =(EtransType)PIn.PLong(table.Rows[i][36].ToString());
		...
		public static long Insert(Claim Cur) {
			...
			if(PrefC.RandomKeys){
				command+="'"+POut.PLong(Cur.ClaimNum)+"', ";
			}
			command+=
				...
				+"'"+POut.PLong   (Cur.PlanNum)+"', "
				+"'"+POut.PLong   (Cur.ProvTreat)+"', "
				...
				+"'"+POut.PLong   (Cur.ProvBill)+"', "
				+"'"+POut.PLong   (Cur.ReferringProv)+"', "
				...
				+"'"+POut.PLong   ((int)Cur.EmployRelated)+"', "
				+"'"+POut.PBool  (Cur.IsOrtho)+"', "
				+"'"+POut.PLong   (Cur.OrthoRemainM)+"', "
				+POut.PDate  (Cur.OrthoDate)+", "
				+"'"+POut.PLong   ((int)Cur.PatRelat)+"', "
				+"'"+POut.PLong   (Cur.PlanNum2)+"', "
				+"'"+POut.PLong   ((int)Cur.PatRelat2)+"', "
				+"'"+POut.PDouble(Cur.WriteOff)+"', "
				+"'"+POut.PLong   (Cur.Radiographs)+"', "
				+"'"+POut.PLong   (Cur.ClinicNum)+"', "
				+"'"+POut.PLong   (Cur.ClaimForm)+"', "
				+"'"+POut.PLong   ((int)Cur.EFormat)+"', "
				+"'"+POut.PLong   (Cur.AttachedImages)+"', "
				+"'"+POut.PLong   (Cur.AttachedModels)+"', "
		...
		public static void Update(Claim Cur){
			...
			string command= "UPDATE claim SET "
				...
				+",plannum = '"        +POut.PLong   (Cur.PlanNum)+"' "
				+",provtreat = '"      +POut.PLong   (Cur.ProvTreat)+"' "
				...
				+",provbill = '"     +	POut.PLong   (Cur.ProvBill)+"' "
				+",referringprov = '"+	POut.PLong   (Cur.ReferringProv)+"' "
				...
				+",employrelated = '" +	POut.PLong   ((int)Cur.EmployRelated)+"' "
				+",isortho = '"       +	POut.PBool  (Cur.IsOrtho)+"' "
				+",orthoremainm = '"  +	POut.PLong   (Cur.OrthoRemainM)+"' "
				+",orthodate = "      +	POut.PDate  (Cur.OrthoDate)+" "
				+",patrelat = '"      +	POut.PLong   ((int)Cur.PatRelat)+"' "
				+",plannum2 = '"      +	POut.PLong   (Cur.PlanNum2)+"' "
				+",patrelat2 = '"     +	POut.PLong   ((int)Cur.PatRelat2)+"' "
				+",writeoff = '"      +	POut.PDouble(Cur.WriteOff)+"' "
				+",Radiographs = '"   + POut.PLong   (Cur.Radiographs)+"' "
				+",ClinicNum = '"     + POut.PLong   (Cur.ClinicNum)+"' "
				+",ClaimForm = '"     + POut.PLong   (Cur.ClaimForm)+"' "
				+",EFormat = '"       + POut.PLong   ((int)Cur.EFormat)+"' "
				+",AttachedImages = '"+ POut.PLong   (Cur.AttachedImages)+"' "
				+",AttachedModels = '"+ POut.PLong   (Cur.AttachedModels)+"' "
			...
			command="DELETE FROM claimattach WHERE ClaimNum="+POut.PLong(Cur.ClaimNum);
			Db.NonQ(command);
		...
		public static void Delete(Claim Cur){
			...
			string command = "DELETE FROM claim WHERE ClaimNum = '"+POut.PLong(Cur.ClaimNum)+"'";
			Db.NonQ(command);
			command = "DELETE FROM canadianclaim WHERE ClaimNum = '"+POut.PLong(Cur.ClaimNum)+"'";
			Db.NonQ(command);
			command = "DELETE FROM canadianextract WHERE ClaimNum = '"+POut.PLong(Cur.ClaimNum)+"'";
			Db.NonQ(command);
		}
		...
		public static ClaimSendQueueItem[] GetQueueList(long claimNum,long clinicNum) {
			...
			if(claimNum==0){
				...
				command+="WHERE claim.ClaimNum="+POut.PLong(claimNum)+" ";
			}
			if(clinicNum>0) {
				command+="AND claim.ClinicNum="+POut.PLong(clinicNum)+" ";
			}
			...
			for(int i=0;i<table.Rows.Count;i++){
				...
				listQueue[i].ClaimNum        = PIn.PLong   (table.Rows[i][0].ToString());
				...
				listQueue[i].PatNum          = PIn.PLong   (table.Rows[i][5].ToString());
		...
		public static object[,] GetX12TransactionInfo(List<long> claimNums) {//ArrayList queueItemss){
			...
			for(int i=0;i<claimNums.Count;i++){
				...
				str.Append(" claim.ClaimNum="+POut.PLong(claimNums[i]));//((ClaimSendQueueItem)queueItems[i]).ClaimNum.ToString());
			}
			string command;
			...
			for(int i=0;i<table.Rows.Count;i++){
				...
				myA[1,i]=PIn.PLong   (table.Rows[i][1].ToString());
				myA[2,i]=PIn.PLong   (table.Rows[i][2].ToString());
				myA[3,i]=PIn.PLong   (table.Rows[i][3].ToString());
				myA[4,i]=PIn.PLong   (table.Rows[i][4].ToString());
			}
			return myA;
		}
