	...
	public class Claims{
		...
		private static List<Claim> SubmitAndFill(DataTable table){
			...
			for(int i=0;i<table.Rows.Count;i++){
				...
				tempClaim.OrthoRemainM  =		PIn.PInt32   (table.Rows[i][27].ToString());
				...
				tempClaim.Radiographs   =   PIn.PInt32   (table.Rows[i][33].ToString());
				...
				tempClaim.AttachedImages=   PIn.PInt32   (table.Rows[i][37].ToString());
				tempClaim.AttachedModels=   PIn.PInt32   (table.Rows[i][38].ToString());
