...
namespace OpenDentBusiness.HL7 {
	...
	public static class MessageConstructor {
		...
		public static MessageHL7 GenerateDFT(List<Procedure> procList,string eventType,string pdfDescription,Patient pat,Patient guar,long aptNum) {//add event (A04 etc) parameters later if needed
					...
					for(int f=0;f<hl7DefMessage.hl7DefSegments[s].hl7DefFields.Count;f++) {
						...
						if(fieldName=="") {//If fixed text instead of field name just add text to segment
							...
							seg.SetField(hl7DefMessage.hl7DefSegments[s].hl7DefFields[f].OrdinalPos,
								FieldConstructor.GenerateDFT(hl7Def,fieldName,pat,prov,procList[repeat],guar,apt,repeat+1,eventType,pdfDescription));
						}
					}
