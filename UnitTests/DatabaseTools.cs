		...
		public static bool SetDbConnection(string dbName,bool isOracle){
			...
			try {
				if(!isOracle) {
					...
					dcon=new OpenDentBusiness.DataConnection(DataConnection.DBtype);
					dcon.SetDb("Data Source=(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=jason)(PORT=1521))(CONNECT_DATA=(SERVER=DEDICATED)(SERVICE_NAME=oracle.opendental.od)));User Id=unittest;Password=unittest;","",DataConnection.DBtype,true);
					RemotingClient.RemotingRole=RemotingRole.ClientDirect;
					return true;
				}
			}
