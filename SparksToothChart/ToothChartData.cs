...
namespace SparksToothChart {
	...
	public class ToothChartData {
		...
		///<summary>Width of entire set of teeth, in mm.  This will soon be replaced.</summary>
		public float WidthProjection;
		public List<ToothInitial> DrawingSegmentList;
		///<summary>When the drawing feature was originally added, this was the size of the tooth chart.  This number must forever be preserved and drawings scaled to account for it.  This is the size of the control in screen pixels.</summary>
		public Size OriginalDrawingSize=new Size(410,307);
		...
		///<summary>Gets the rectangle surrounding a tooth number.  Used to draw the box and to invalidate the area.  widthControl is a stub that will soon be replaced.</summary>
		public RectangleF GetNumberRecMm(string tooth_id,string displayNum,float strWidthMm,int widthControl) {
			float xPos=0;
			float yPos=0;
		...
		public float GetTransXpix(string tooth_id,int widthControl) {
			...
			return (WidthProjection/2f+xmm)*widthControl/WidthProjection;
		}

		///<summary>In control coords rather than mm.</summary>
		public float GetTransYfacialPix(string tooth_id,int heightControl) {
			float basic=30;
			if(ToothGraphic.IsMaxillary(tooth_id)) {
				return heightControl/2-basic;
			}
			return heightControl/2+basic;
		}
		...
		public Rectangle ConvertRecToPix(RectangleF recMm,int widthControl,int heightControl) {
			float toMm=(float)WidthProjection/(float)widthControl;//mm/pix
