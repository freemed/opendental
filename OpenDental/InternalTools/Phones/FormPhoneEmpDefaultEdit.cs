...
namespace OpenDental{
	...
	public class FormPhoneEmpDefaultEdit:System.Windows.Forms.Form {
		...
		private void InitializeComponent()
		{
			...
			this.checkNoGraph.Click += new System.EventHandler(this.checkNoGraph_Click);
			// 
			// label5
			// 
			...
			this.checkNoColor.Click += new System.EventHandler(this.checkNoColor_Click);
			// 
			// textEmpName
			// 
    ...
    "t match the extension.  Not used by floaters.";
		...
		private void checkNoGraph_Click(object sender,EventArgs e) {
			if(Security.IsAuthorized(Permissions.Schedules)) {
				return;
			}
			//Put the checkbox back the way it was before user clicked on it.
			if(checkNoGraph.Checked) {
				checkNoGraph.Checked=false;
			}
			else {
				checkNoGraph.Checked=true;
			}
		}

		private void checkNoColor_Click(object sender,EventArgs e) {
			if(Security.IsAuthorized(Permissions.Schedules)) {
				return;
			}
			//Put the checkbox back the way it was before user clicked on it.
