	...
	//    }
		...
		public static void SendData(Program ProgramCur, Patient pat)
		{
			...
			if (pat != null)
			{
				...
				htKeyVals["LastName"] = pat.LName;  //			WriteValue("LastName", pat.LName);
				htKeyVals["FirstName"] = pat.FName; //			WriteValue("FirstName", pat.FName);
				//Patient Id can be any string format.
				PPCur = ProgramProperties.GetCur(ForProgram, "Enter 0 to use PatientNum, or 1 to use ChartNum");
				if (PPCur.PropertyValue == "0")
				{
					htKeyVals["PatientID"] = pat.PatNum.ToString();  //WriteValue("PatientID", pat.PatNum.ToString());

				}
				else
				{
					htKeyVals["PatientID"] = pat.ChartNumber; ;		// WriteValue("PatientID", pat.ChartNumber);

				}
				htKeyVals["PatientSSN"] = pat.SSN;	// WriteValue("PatientSSN", pat.SSN);
				//WriteValue("SubscriberSSN",pat);
				if (pat.Gender == PatientGender.Female)
					htKeyVals["Gender"] = "Female";		// WriteValue("Gender", "Female");
				else
					htKeyVals["Gender"] = "Male";		//WriteValue("Gender", "Male");
				htKeyVals["DOB"] = pat.Birthdate.ToString("MM/dd/yy");	// WriteValue("DOB", pat.Birthdate.ToString("MM/dd/yy"));
				//WriteValue("ClaimID",pat);??huh
				htKeyVals["AddrStreetNo"] = pat.Address;	// WriteValue("AddrStreetNo", pat.Address);
				//WriteValue("AddrStreetName",pat);??
				//WriteValue("AddrSuiteNo",pat);??
				htKeyVals["AddrCity"] = pat.City;	//WriteValue("AddrCity", pat.City);
				htKeyVals["AddrState"] = pat.State; //WriteValue("AddrState", pat.State);
				htKeyVals["AddrZip"] = pat.Zip; //	WriteValue("AddrZip", pat.Zip);
				htKeyVals["PhHome"] = LimitLength(pat.HmPhone, 13); //WriteValue("PhHome", LimitLength(pat.HmPhone, 13));
				htKeyVals["PhWork"] = LimitLength(pat.WkPhone, 13); //WriteValue("PhWork", LimitLength(pat.WkPhone, 13));
