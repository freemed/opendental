...
namespace xCrudGeneratorWebService {
	public partial class Form1:Form {
		...
		private void Form1_Load(object sender,EventArgs e) {
			...
			JavaTableTypesDir=@"..\..\..\OpenDentalWeb\OpenDentBusiness\src\com\opendental\opendentbusiness\tabletypes";
			JavaSerializingDir=@"..\..\..\OpenDentalWeb\OpenDentBusiness\src\com\opendental\opendentbusiness\remoting\Serializing.java";
		...
		private void StartJavaSerial(StringBuilder strb) {
			...
			strb.Append("package com.opendental.opendentbusiness.remoting;"+rn+rn
				...
				+"import com.opendental.opendentbusiness.data.*;"+rn
				+"import com.opendental.opendentbusiness.tabletypes.*;"+rn+rn
		...
		private void EndJavaSerial(StringBuilder strb) {
							...
							+t4+"if(i<nodeListObjects.size()) {"+rn
								...
								+t5+"try {"+rn
									+t6+"getDeserializedObject(nodeListObjects.get(i).toString(),new DeserializeCallbackResult() {"+rn
										...
										+t7+"}"+rn
										+t7+"public void onError(String error) {"+rn
											+t8+"//Not sure how to have the error message passed on.  Do nothing for now."+rn
										+t7+"}"+rn
									+t6+"});"+rn
								+t5+"}"+rn
			...
			strb.Append(t+"/** The sole purpose of this interface is to return a result to the Db class after the repeating command has finished.  The Db class should be the only class listening to this callback. */"+rn
				+t+"public interface DeserializeCallbackResult {"+rn
					...
					+t2+"void onError(String error);"+rn
				+t+"}"+rn+rn
				+"}"+rn+rn);
			#endregion
		}
		...
		private void WriteAlljavaTableTypes(StringBuilder strb,string className,FieldInfo[] fields) {
			...
			#region class header
			strb.Append("package com.opendental.opendentbusiness.tabletypes;"+rn
				+rn+"import com.google.gwt.xml.client.Document;"+rn
				+"import com.opendental.opendentbusiness.remoting.Serializing;"+rn);
			bool hasDate=false;
			bool hasList=false;
